...
export default function RecentPosts({ blogs }: RecentPostsProps) {
  ...
  // Prevent hydration mismatch by not rendering search until mounted
  if (!isMounted) {
    return (
      <section className="blog-grid-section w-full mx-auto py-12 sm:py-16 px-4 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="blog-section-title text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-6 sm:mb-8">
            Alle Artikel
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {blogs.map((blog) => (
              <article key={blog.slug} className="blog-card bg-white rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.06)] overflow-hidden border border-gray-100">
                ...
                <div className="blog-card-content p-4 sm:p-6">
                  <h3 className="blog-card-title text-lg sm:text-xl font-semibold mb-2 sm:mb-3 line-clamp-2 text-gray-900">
                    {blog.title}
                  </h3>
  ...
  return (
    <section className="blog-grid-section w-full mx-auto py-12 sm:py-16 px-4 bg-gray-50">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Search and Filter - Only show after mount to prevent hydration issues */}
        {isMounted && <BlogSearch blogs={blogs} onFilteredBlogs={setFilteredBlogs} />}
